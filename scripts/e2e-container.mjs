...
async function ensureOpenClawMessageRoutes() {
  const wsResponse = await fetch(`${baseURL}/v1/openclaw/messages/ws`);
  if (wsResponse.status !== 404) {
    return;
  }
  throw new Error(
    `moltenhub image "${moltenhubImage}" does not expose /v1/openclaw/messages/ws; set MOLTENHUB_IMAGE to a build containing OpenClaw message routes`
  );
}
...
async function main() {
  ...
  await waitForHealth();
  await ensureOpenClawMessageRoutes();
